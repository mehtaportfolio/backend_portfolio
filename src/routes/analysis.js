...
import { getAnalysisDashboard, getAnalysisSummary, getAnalysisFreeStocks, getTopMutualFunds, getEarningData, getTodayTopGainersLosersDayChange } from '../services/analysisService.js';
...
router.get('/dashboard', cacheMiddleware(ANALYSIS_CACHE_TTL), async (req, res, next) => {
  try {
    ...
    const data = await getAnalysisDashboard(priceSource);
    res.json(data);
  } catch (error) {
    next(error);
  }
...
router.get('/account-dashboard', cacheMiddleware(ANALYSIS_CACHE_TTL), async (req, res, next) => {
  try {
    ...
    const data = await getAccountAnalysis(supabase, priceSource);
    res.json({
      success: true,
      data,
...
router.get('/summary', cacheMiddleware(ANALYSIS_CACHE_TTL), async (req, res, next) => {
  try {
    ...
    const data = await getAnalysisSummary(priceSource);
    res.json(data);
  } catch (error) {
    next(error);
  }
...
router.get('/free-stocks', cacheMiddleware(ANALYSIS_CACHE_TTL), async (req, res, next) => {
  try {
    ...
    const data = await getAnalysisFreeStocks(priceSource);
    res.json(data);
  } catch (error) {
    next(error);
  }
...
router.get('/fixed-assets', authMiddleware, cacheMiddleware(ANALYSIS_CACHE_TTL), async (req, res, next) => {
  try {
    const data = await getFixedAssetTotals(supabase);
    res.json({
      success: true,
      data,
      cache: res.getHeader('X-Cache'),
...
// Today top gainers/losers by day-change (cmp-lcp), for open stock positions
router.get('/today-top-gainers-losers-daychange', cacheMiddleware(ANALYSIS_CACHE_TTL), async (req, res, next) => {
  try {
    const priceSource = req.query.priceSource || 'stock_master';
    const data = await getTodayTopGainersLosersDayChange({ supabase, priceSource });
    res.json(data);
  } catch (error) {
    next(error);
  }

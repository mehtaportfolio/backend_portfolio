 ...
 */
export async function startCorpActionService(days = 30) {
  const startTime = Date.now();
  console.log(`\n🔔 [Corporate Action Service] Starting Master Sync for ${days} days...`);

  try {
    // Run scrapers in sequence to manage load
    console.log("\n📡 --- NSE Scraper ---");
    await runNSEActions(days);
  } catch (err) {

...
export async function sendTelegramAlert(payload) {
  ...
  try {
    ...
    const message = escapedTitle ? `<b>${escapedTitle}</b>\n\n${escapedBody}` : escapedBody;
...
/**
 * Format price: 1 decimal place if < 100, else integer
 * @param {number} price 
 * @returns {string}
 */
...
export async function sendPushNotification(payload) {
  try {
    const { data: subscriptions, error } = await fetchAllRows(supabase, 'push_subscriptions', {
      ...
      webpush.sendNotification(sub.subscription, notificationPayload)
        .catch(err => {
          if (err.statusCode === 410 || err.statusCode === 404) {
            ...
            return deleteRows(supabase, 'push_subscriptions', (q) => q.eq('id', sub.id));
          }
...
async function fetchNotificationSettings() {
  try {
    const { data: userDetailsArr, error } = await fetchAllRows(supabase, 'user_details', {
      filters: [(q) => q.eq('id', 1)],
      limit: 1
    });
...
export async function triggerPortfolioUpdate(force = false, threshold = null, type = 'all') {
  ...
  try {
    // Single-user: fetch all asset data
    const result = await getDashboardAssetAllocation(supabase);
    ...
    const stockDayChange = stockRows.reduce((sum, r) => sum + r.dayChange, 0);
    const stockMarketValue = stockRows.reduce((sum, r) => sum + r.marketValue, 0);
    const stockDayChangePercent = (stockMarketValue - stockDayChange) > 0 
      ? (stockDayChange / (stockMarketValue - stockDayChange)) * 100 
      : 0;
    ...
    const etfDayChange = etfRows.reduce((sum, r) => sum + r.dayChange, 0);
    const etfMarketValue = etfRows.reduce((sum, r) => sum + r.marketValue, 0);
    const etfDayChangePercent = (etfMarketValue - etfDayChange) > 0 
      ? (etfDayChange / (etfMarketValue - etfDayChange)) * 100 
      : 0;
    ...
    const dayStr = `|| Day: ₹${overallDayChange.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${dayChangePercent.toFixed(1)}%)`;
    const stockDayStr = `|| Day: ₹${stockDayChange.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${stockDayChangePercent.toFixed(1)}%)`;
    const etfDayStr = `|| Day: ₹${etfDayChange.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${etfDayChangePercent.toFixed(1)}%)`;

    let mobileBody = `Equity P&L: ₹${totalProfit.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${profitPercent.toFixed(1)}%) ${dayStr}`;
    mobileBody += `\nStocks: ${stockProfit.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${stockProfitPercent.toFixed(1)}%) ${stockDayStr}`;
    mobileBody += `\nETF: ${etfProfit.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${etfProfitPercent.toFixed(1)}%) ${etfDayStr}`;

    let telegramBody = `Equity P&L: ₹${totalProfit.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${profitPercent.toFixed(1)}%) ${dayStr}`;
    telegramBody += `\nStocks: ${stockProfit.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${stockProfitPercent.toFixed(1)}%) ${stockDayStr}`;
    telegramBody += `\nETF: ${etfProfit.toLocaleString('en-IN', { maximumFractionDigits: 0 })} (${etfProfitPercent.toFixed(1)}%) ${etfDayStr}`;
    ...
    if (highProfitRegularStocks.length > 0) {
      ...
      sortedAccountKeys.forEach(accountKey => {
        ...
        stocks.forEach(s => {
          const line = `\n• ${s.stockName}: ${s.profitPercent.toFixed(0)}% (C:${formatPrice(s.cmp)}, A:${formatPrice(s.avgBuyPrice)})`;
          mobileBody += line;
          telegramBody += line;
        });
      });

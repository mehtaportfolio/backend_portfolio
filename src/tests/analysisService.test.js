...
const { getAnalysisSummary } = await import('../services/analysisService.js');
...
describe('getAnalysisSummary MF Aggregation', () => {
  ...
  test('handles partial exits - active and closed portions', async () => {
    ...
    fetchAllRows
      ...
      .mockImplementationOnce(() => Promise.resolve({ data: [] })) // priceData
    ...
    // The current implementation aggregates by scheme and only emits active if no closed lots exist for that scheme
    // or aggregates them differently.
  ...
  test('handles multiple accounts per scheme', async () => {
    ...
    // Implementation aggregates across accounts for the same scheme
    expect(result.mfActive).toHaveLength(1); 
    expect(result.mfClosed).toHaveLength(0);

    const active = result.mfActive[0];
    expect(active.units).toBe(200);
    expect(active.invested).toBe(2000);
  });

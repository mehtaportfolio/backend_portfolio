...
async function getPortfolioCodes() {
  try {
    ...
    const uniqueStockNames = [...new Set(data.map(r => r.stock_name).filter(Boolean))];
    if (uniqueStockNames.length === 0) return [];

    const { data: master, error: masterError } = await fetchAllRows(supabase, "stock_master", {
...
async function fetchActionsForCode(code, range, cookie) {
  ...
  try {
    ...
    if (typeof data === 'string' && data.includes("Access Denied")) {
      return [];
    }
...
export async function runBSEActions(days = 30) {
  console.log(`🚀 Starting BSE corporate action scraper (period: ${days} days)...`);
  try {
